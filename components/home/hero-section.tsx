...
import { ArrowRight, GraduationCap, FileCheck, Briefcase, Sparkles, ChevronDown } from "lucide-react";
...
export default function HeroSection() {
  ...
  return (
    ...
    >
      ...
      <div className="sticky top-32 w-full z-20 px-6">
        <motion.div style={{ opacity, scale, y }} className="max-w-5xl mx-auto text-center">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-6">
            <Button size="lg" className="h-16 px-12 bg-red-600 hover:bg-red-700 text-white font-black uppercase tracking-[0.2em] text-xs rounded-full transition-all hover:scale-105 shadow-xl shadow-red-900/20">
              Start Your Journey
              <ArrowRight className="ml-2 w-5 h-5" />
            </Button>
            <Button variant="ghost" className="h-16 px-8 text-white/60 hover:text-white font-black uppercase tracking-widest text-xs">
              View Our Methods
            </Button>
          </div>
      ...
      <motion.div 
        animate={{ 
          backgroundColor: index === 0 ? "rgba(245, 158, 11, 0.1)" : index === 1 ? "rgba(220, 38, 38, 0.1)" : "rgba(255, 255, 255, 0.05)" 
        }}
